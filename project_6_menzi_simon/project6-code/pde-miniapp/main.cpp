...
void write_binary(std::string fname, Field& u, SubDomain& domain,
    ...
    int result = MPI_File_open(domain.comm_cart, fname.c_str(),
    ...
    result = MPI_Type_create_subarray(
        2, dimuids, ucount, ustart, MPI_ORDER_FORTRAN, MPI_DOUBLE, &filetype);
    assert(result == MPI_SUCCESS);

    result = MPI_Type_commit(&filetype);
    assert(result == MPI_SUCCESS);

    result = MPI_File_set_view(filehandle, disp, MPI_DOUBLE, filetype,
                               "native", MPI_INFO_NULL);
    assert(result == MPI_SUCCESS);

    result = MPI_File_write_all(filehandle, u.data(), domain.N, MPI_DOUBLE,
                                MPI_STATUS_IGNORE);
    assert(result == MPI_SUCCESS);
...
static void readcmdline(Discretization& options, int argc, char* argv[]) {
    if (argc < 4 || argc > 5) {
        ...
        std::cerr << "  nx  number of gridpoints in x-direction and "
...
int main(int argc, char* argv[]) {
    ...
    // initialize
    // use "MPI_Comm_size", "MPI_Comm_rank" and "MPI_Init"
    // (you may also use MPI_Init_thread)
    MPI_Init(&argc, &argv);
    ...
    if (domain.rank == 0) {
        ...
        std::cout << "                      Welcome to mini-stencil!"
                  << std::endl;
        std::cout << "version   :: MPI : " << domain.size << " MPI ranks"
                  << std::endl;
        std::cout << "mesh      :: " << options.nx << " * " << options.ny
    ...
    for (int j = domain.starty - 1; j < domain.endy; j++) {
        ...
        for (int i = domain.startx - 1; i < domain.endx; i++) {
            ...
            if ((x - xc) * (x - xc) + (y - yc) * (y - yc) < radius * radius)
                y_new(i - domain.startx + 1, j - domain.starty + 1) =
                    inner_circle;
        }
    }
    ...
    if (domain.rank == 0) std::cout << "Goodbye!" << std::endl;

    // finalize it using "MPI_Finalize" and "MPI_Comm_free"
